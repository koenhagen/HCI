...
void loadData() {
  ...
  for (int i = 0; i<t.getRowCount(); i++) { // Access each row of the table one at a time, in a loop.
    ...
    //units[i] = new Unit(random(width), random(height), 30);
  }
}
...
void display() {
  for (int i = 0; i<t.getRowCount(); i++) {
    ...
    ellipse(/* x value*/random(r.getFloat("Totaal"), width - r.getFloat("Totaal")),/* y value*/ random(r.getFloat("Totaal"), height - r.getFloat("Totaal")), r.getFloat("Totaal"), r.getFloat("Totaal") ); // ellipse(random(diam, width - diam, etc.)
  }
}
